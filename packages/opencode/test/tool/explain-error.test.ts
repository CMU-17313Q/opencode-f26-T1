...
describe("tool.explain_error", () => {
  it.instance("explains the latest failure with evidence before guidance and no fix", () =>
    Effect.gen(function* () {
      const latest = failure("error: expect(received).toBe(4)\n    at add (/repo/src/math.test.ts:8:18)")
      const result = yield* explain(undefined, [request, failure("ENOENT: old.ts"), latest])
